...
import { POLYGON_AMOY, CHAIN_IDS, DEFAULT_RPC_URL } from '../config/chains';
...
export class SafeService {
  ...
  constructor(chainId: number = CHAIN_IDS.POLYGON_AMOY, rpcUrl: string = DEFAULT_RPC_URL) {
  ...
  private getContractNetworks(): any | undefined {
    ...
    if (this.chainId !== CHAIN_IDS.POLYGON_AMOY) return undefined;
  ...
  private async predictSafeAddressUsingProtocolKit(stealthAddress: string, saltNonce: string = '0'): Promise<string> {
    try {
      Logger.info('Predicting Safe address using Protocol Kit', {
        ...
        saltNonce,
        ...(this.chainId === CHAIN_IDS.POLYGON_AMOY ? { contractNetworks: this.getContractNetworks() } : {})
      });
  ...
  static getSupportedNetworks(): Array<{ chainId: number; name: string; rpcUrl: string }> {
    return [
      { chainId: CHAIN_IDS.POLYGON_AMOY, name: 'Polygon Amoy Testnet', rpcUrl: 'https://rpc-amoy.polygon.technology/' }
    ];
  }
